...
(function () {
  ...
  function render(container, config, text) {
    ...
    var stripeTitle = get(text, 'fiat.stripe.title');
    if (stripeTitle === undefined) stripeTitle = 'card (stripe)';
    var stripeDesc = get(text, 'fiat.stripe.description');
    if (stripeDesc === undefined) stripeDesc = 'donate securely with any major credit or debit card.';
